...
//Construtor parametrizado com velocidade igual a 100
...
//Construtor parametrizado com velocidade igual a 100 e trem começando de outra parte do quadrado
Trem::Trem(int ID, int x, int y, int xtrem, int ytrem){
    this->ID = ID;
    this->x = xtrem;
    this->y = ytrem;
    this->xinicio = x;
...
//construtor com velocidade indicada
        ...
        case 1://Trem 1
          while(true){
            if (y == yinicio && x <(xinicio+270)){//direita
                ...
                deadlock[3]->acquire();
                deadlock[2]->acquire();
                std::cout << "trem1 pegou o deadlock 2 " << deadlock[2]->available() << std::endl;
                critic0.lock();
        ...
        case 2: //Trem 2
          while(true){
              ...
              if (x > xinicio && y == 150){//esquerda
                  ...
                  deadlock[2]->acquire();
                  std::cout << "trem2 pegou o deadlock 2 " << deadlock[2]->available() << std::endl;
                  critic3.lock();
        ...
        case 4: //Trem 4
          while(true){
              ...
              if (x == 540 && y < 270 ){//desce
                  ...
                  std::cout << "trem4 soltou o deadlock 2 " << deadlock[2]->available() << std::endl;
              ...
              if(x==xinicio && y > yinicio){//sobe
                  ...
                  deadlock[2]->acquire();
                  std::cout << "trem4 pegou o deadlock 2 " << deadlock[2]->available() << std::endl;
                  critic2.lock();
...
void Trem::mudaVelocidade(int v){
    int newVelocity = 200-v+10;
    this->velocidade = newVelocity;
    mudaStop(newVelocity >=210 );

}

void Trem::mudaStop(bool b){
    this->isStopped = b;
}
